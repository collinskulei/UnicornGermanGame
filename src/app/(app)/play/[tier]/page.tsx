...
export default function TierDeckListPage({ params }: PageProps<"/play/[tier]">) {
  ...
  const decks = getPuzzlesForTier(tierId);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {decks.map((deck, i) => {
          const result = best[deck.id];
          return (
            <Link
              key={deck.id}
              href={`/play/${tierId}/${deck.id}`}
            ...
            >
              ...
              <p className="font-display text-lg font-bold text-unicorn-blue">{deck.theme}</p>
              <p className="text-sm text-ink-soft">{deck.words.length} cards</p>
              {result && !loading && (
