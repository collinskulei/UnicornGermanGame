...
import { useCrossword } from "@/lib/game/useCrossword";
...
export default function PuzzleGamePage({ params }: PageProps<"/play/[tier]/[puzzleId]">) {
  ...
  // Keyed by puzzle id: navigating "More Puzzles" → a different puzzle
  // reuses this route without an unmount, so the key forces a fresh
  // PuzzleGame instance (fresh grid/timer/hints) instead of carrying
  // over the previous puzzle's game state.
  return <PuzzleGame key={puzzle.id} tier={tier} puzzle={puzzle} isDaily={isDaily} />;
}

function PuzzleGame({ tier, puzzle, isDaily }: { tier: TierId; puzzle: Puzzle; isDaily: boolean }) {
  const { user, refreshProfile } = useAuth();
  const { state, actions } = useCrossword(puzzle);
  ...
  return (
    <div className="flex flex-col items-center gap-6 pb-24">
      ...
      <CrosswordBoard puzzle={puzzle} state={state} actions={actions} />

      <button
        type="button"
        onClick={actions.useHint}
        className="rounded-full bg-cloud px-4 py-2 text-sm font-semibold text-unicorn-blue shadow-card transition hover:bg-unicorn-orange-light"
      >
        💡 Reveal a letter ({state.hintsUsed} used)
      </button>

      <ClueList puzzle={puzzle} state={state} actions={actions} />
      ...
      {state.isComplete && (
        ...
        >
          <div className="flex w-full max-w-lg flex-col items-center gap-2 text-center">
            <p className="font-display text-2xl font-bold text-unicorn-blue">
              🎉 Puzzle Complete!
            </p>
